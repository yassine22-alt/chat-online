            ...
            <div class="card" id="chat2">
              ...
              >
                ...
                <div class="typing-indicator" v-if="typingUsers.length > 0">
                  <p v-for="user in typingUsers" :key="user.id">
                    {{ user.name }} is typing...
                  </p>
                </div>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      currentUserAvatar: "", // Assuming you have logic to set the current user's avatar
      chatName: "",
      chatroomAvatar: "", // Added for chatroom/group avatar
    };
  },
  created() {
        ...
        if (chatroomData.message) {
          this.messages = await Promise.all(
            chatroomData.message.map(async (messageId) => {
              ...
              return { id: messageDoc.id, senderAvatar, ...messageData };
            })
          );
          this.scrollToEnd();
        }
